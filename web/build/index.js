    ...
    (function () {
        var TopBtn = React.createClass({displayName: "TopBtn",
            ...
            render: function () {
                ...
                return React.createElement("a", {href: "#"}, React.createElement("span", {className: btnClass, onClick: self.props.onClick}, React.createElement("i", {
                    className: iconClass}), self.props.title))
            }
        });
        ...
        filterBtn = React.render(React.createElement(TopBtn, {icon: "uk-icon-filter", title: "过滤", 
                                         onClick: filterBtnClick}), document.getElementById("J_filterBtnContainer"));
        ...
        //safe util
        var safeBtn,
            SafePanel = PopupContent["safe"],
            safePanelEl;
        safeBtn = React.render(React.createElement(TopBtn, {icon: "uk-icon-filter", title: "安全", 
                                       onClick: safeBtnClick}), document.getElementById("J_safeBtnContainer"));
        safePanelEl = (React.createElement(SafePanel, null))
        //打开面板
        function safeBtnClick() {
            showPop({left: "60%", content: safePanelEl});
        }
        ...
        require("./mapPanel").fetchConfig(function (initConfig) {
            ...
            mapBtn = React.render(React.createElement(TopBtn, {icon: "uk-icon-shield", title: "本地映射", 
                                          onClick: mapBtnClick}), document.getElementById("J_filterContainer"));
